...
function searchTrackAlbum($id){
    ...
    $result = $sql->fetchArray(SQLITE3_NUM);
        ...
        elseif (!$result) { 
            ...
            searchAlbumTrackSpotify($id);
        }

    } 
    
    
    function findTrackAlbum($nom_album,$nom_track){
        $db = new SQLite3('../wp-content/plugins/spotify/spotify_db.db'); 
        ...
        $result = $sql->fetchArray(SQLITE3_NUM);
            if ($result) {
                ...
                $result2 = $sql2->fetchArray(SQLITE3_NUM);
                    if ($result2) {
                        displayTrackPage($nom_track,$result[0]);
                    }elseif (!$result2) { 
                        searchTrackAlbum($nom_album);
                    }                
            }
            elseif (!$result) { 
                 searchAlbum($nom_album);
            }    
        }


function searchAlbumTrackSpotify($id){
    $db = new SQLite3('../wp-content/plugins/spotify/spotify_db.db');
    $token=getToken();
...
function searchTrackArtist($id_artist){
    ...
    if($result){
        ...
        searchAlbumArtistOnSpotify($result[0]);
    }
}
...
function searchTrack($track){
    ...
    $result = $sql->fetchArray(SQLITE3_NUM);
    if($result){
        displayTrackPage($track,$result[0]);
    }elseif(!$result){

        searchTrackSpotify($track);
    }
}

 function searchTrackSpotify($track){
     ...
     $db = new SQLite3('../wp-content/plugins/spotify/spotify_db.db');
         ...
         $album = $api->searchTrack($name); //recherche de l'abum via l'api. La fonction   searchAlbum n'était pas présente dans la librérie, nous l'avons devellope
...
function searchOneTrackArtist($artist,$track){
    $db = new SQLite3('../wp-content/plugins/spotify/spotify_db.db');
